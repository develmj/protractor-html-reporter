...
var HTMLReport = function() {
	...
	for (var i=0; i<totalSuites; i++) {
		...
		if (testCasesResults[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
			...
			screenshotsNamesOnFailure.push(report.deviceName +'-'+ report.browser +'-'+ suite.name.substring(suite.name.indexOf(".")+1) + ' ' + testCasesNames[j] + '.png');
			...
			screenshotsNamesOnFailure.push(report.deviceName +'-'+ report.browser +'-'+ suite.name + ' ' + testCasesNames[j] + '.png');
		    }
		}
	    }
	...
	report.deviceName = testConfig.deviceName || 'unknownDevice';
	...
	//write to html file
	var testOutputPath = getPath('/' + report.deviceName + '-' + report.browser + '-test-report.html', testConfig['outputPath'] || './');
