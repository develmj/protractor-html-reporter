...
var HTMLReport = function() {
	...
	for (var i=0; i<totalSuites; i++) {
		...
		if (testCasesResults[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
			...
			screenshotsNamesOnFailure.push(report.devicePrintName +'-'+ report.browser +'-'+ suite.name.substring(suite.name.indexOf(".")+1) + ' ' + testCasesNames[j] + '.png');
			...
			screenshotsNamesOnFailure.push(report.devicePrintName +'-'+ report.browser +'-'+ suite.name + ' ' + testCasesNames[j] + '.png');
		    }
		}
	    }
	...
	report.devicePrintName = testConfig.devicePrintName || 'unknownDevice';
	...
	//write to html file
	var testOutputPath = getPath('/' + report.devicePrintName + '-' + report.browser + '-test-report.html', testConfig['outputPath'] || './');
